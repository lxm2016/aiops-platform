...
有值的列出来, 一眼就能认出:
    ...
    0x0000 = 256  ->  256/10 = 25.6 ℃   多半是温度
    0x0001 = 634  ->  634/10 = 63.4 %   多半是湿度
...
def guess_meaning(addr, fc, raw):
    ...
    温湿度传感器(建大仁科 RS-WS-N01 这类)的通用约定:
      保持寄存器 0x0000 = 温度 ×10, 0x0001 = 湿度 ×10
    ...
    tips = []
    dec = raw / 10.0
    # 温湿度: 一般是前两个寄存器, 一温度一湿度
    if addr == 0 and 0 < dec <= 120:
        tips.append(f"×0.1 → {dec:.1f} ℃（温湿度探头: 0x0000=温度）")
    elif addr == 1 and 0 < dec <= 100:
        tips.append(f"×0.1 → {dec:.1f} %（温湿度探头: 0x0001=湿度）")
...
def main():
    ...
    if any(n for _, n in summary):
        ...
        print("  · 温度 25.6 ℃  →  地址 0x0000, 系数 0.1")
        print("  · 湿度 63.4 %  →  地址 0x0001, 系数 0.1")
